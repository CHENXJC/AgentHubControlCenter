...
AGENTHUB_CURRENT_CHECKPOINT = "HUB-V2-024-DEEP-CHINESE-UI-COVERAGE-CHECK-COMPLETE"
...
def build_agent_manifest(
    ...
    return {
        ...
        "display_name_zh": agent.get("display_name_zh", "").strip(),
        ...
        "dashboard_path": agent.get("dashboard_path", "").strip(),
        "report_path": agent.get("report_path", "").strip(),
        "agenthub_summary_path": agent.get("agenthub_summary_path", "").strip(),
        "contract_path": agent.get("contract_path", "").strip(),
        "checkpoint": agent.get("checkpoint", "").strip(),
        "local_first": agent.get("local_first") if isinstance(agent.get("local_first"), bool) else True,
        "public_safe": agent.get("public_safe") if isinstance(agent.get("public_safe"), bool) else True,
...
def build_agent_contract() -> dict:
    ...
    return {
        ...
        "purpose": "Standard local-first declaration for AgentHubControlCenter tools, actions, Codex prompts, useful signals, connector readiness simulations, local workflow simulations, approval gates, public-safe demo workflow report exports, showcase screenshots, public release readiness metadata, bilingual UI stage sync, and deep Chinese UI display coverage.",
        ...
        "execution_policy": {
            "current_stage": "deep_chinese_ui_coverage_check",
        ...
        "connector_policy": {
            "current_stage": "deep_chinese_ui_coverage_check",
            "live_connectors": "not_enabled_in_hub_v2_024",
